...
- `/log/[slug]`: `BlogPosting` + `BreadcrumbList`（author・publisher に `FlagshipWorks合同会社`）
...
**ロゴ**
- `src/lib/ogLogo.tsx` の `<LogoOg opacity={0.5} />` を使用
- フォントは Google Fonts API から TTF を取得（`curl/7.64.1` UA で fetch → `src: url(...)` を正規表現で抽出）
- Satori（next/og のレンダラー）は woff2 非対応のため、必ず TTF を使用すること
