...
### ローンチパートナー募集の構成
- VACANCYバナー: 左に残り枠数（大きな数字）、右に緑の点滅ドット（`animate-ping`）+ 「OPEN」テキスト
- BENEFITS: 3カードを横並び（制作費20%OFF・戦略コンサル・医師フル監修）。各カードにCheckCircle2アイコン
- REQUIREMENTS: 応募条件3項目（控えめなスタイル。`text-xs text-foreground/40`）
- CTA: 「無料相談・ローンチパートナー応募はこちら」（中央配置）

### アクティブステータスの表現
- 緑の点滅ドット: `animate-ping` を使用。外側リングは `bg-green-400 opacity-60`、内側は `bg-green-400`
- 「OPEN」テキスト: `text-green-400/80`
