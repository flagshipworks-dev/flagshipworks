...
### 料金プランの構成
- **Standard Plan**: ¥600,000〜（税別）。MEDICAL CHECK付き。ページ制作5〜8枚・スマホ最適化・フォーム・SEO・アナリティクス含む
- **Professional Plan**: ¥1,200,000〜（税別）。MEDICAL CHECK + STRATEGY付き。ページ制作10〜15枚・写真撮影ディレクション・ロゴデザイン追加

### ローンチパートナー募集の構成
- VACANCY表示（残り枠数を大きく表示）
- BENEFITS: 3カードを横並び（制作費40%OFF・戦略コンサル・医師フル監修）
- REQUIREMENTS: 応募条件3項目（控えめなスタイル）
- CTA: 「無料相談・ローンチパートナー応募はこちら」
...
| `/service/medical-web` | 医療業界向けサイト制作 詳細ページ（特徴・対象・料金プラン・ローンチパートナー募集・制作実績・関連ログ・CTA） |
| `/works` | 制作実績一覧 |
| `/works/[slug]` | 制作実績詳細 |
| `/log` | ログ一覧 |
...
- `/service/medical-web`: `Service`
- `/log/[slug]`: `BlogPosting`（author・publisher に `FlagshipWorks合同会社`）
